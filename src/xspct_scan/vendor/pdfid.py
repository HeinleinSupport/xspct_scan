...
class cEntropy:
    ...
    def calc(self):
        self.nonStreamBucket = list(
            map(operator.sub, self.allBucket, self.streamBucket)
        )
...
class cPDFiD:
    def __init__(self, xmlDoc, force):
        ...
        self.errorOccured = (
            xmlDoc.documentElement.getAttribute("ErrorOccured") == "True"
        )
    ...
    if options.select:
        if options.force or not oPDFiD.errorOccured and oPDFiD.isPDF:
            ...
            try:
                ...
                Print(
                    "Error evaluating select expression: %s" % options.select, options
                )
                if options.verbose:
                    raise e
                return
        ...
        for cPlugin in plugins:
            if not cPlugin.onlyValidPDF or not oPDFiD.errorOccured and oPDFiD.isPDF:
                ...
                if options.csv:
                    ...
                    if score >= options.minimumscore:
                        Print(
                            PDFiD2String(xmlDoc, options.nozero, options.force), options
                        )
                        Print("%s score:        %.02f" % (cPlugin.name, score), options)
                        try:
                            Print(
                                "%s instructions: %s"
                                % (cPlugin.name, oPlugin.Instructions(score)),
                                options,
                            )
...
class cExpandFilenameArguments:
    def __init__(
        ...
        if len(filenames) == 0:
            ...
            for dirwildcard in filenames:
                if expressionprefix != None and dirwildcard.startswith(
                    expressionprefix
                ):
            ...
            self.warning = (
                ...
                and len(self.filenameexpressions) == 0
            )
            if self.warning:
    ...
    def Glob(self, filename):
        if not (
            "?" in filename or "*" in filename or ("[" in filename and "]" in filename)
        ):
            return [filename]
    ...
    def CheckIfFilesAreValid(self):
        ...
        for filename, expression in self.filenameexpressions:
            ...
            try:
                hashfile = (
                    FilenameCheckHash(filename, self.literalfilenames)[0] == FCH_DATA
                )
            except:
                pass
        ...
        if len(doesnotexist) > 0:
            self.warning = True
            self.message += (
                "The following files do not exist and will be skipped: "
                + " ".join(doesnotexist)
                + "\n"
            )
        if len(isnotafile) > 0:
            self.warning = True
            self.message += (
                "The following files are not regular files and will be skipped: "
                + " ".join(isnotafile)
                + "\n"
            )
...
def PDFiDMain(filenames, options):
    ...
    if options.csv:
        if plugins != []:
            Print(
                MakeCSVLine(
                    (("%s", "Filename"), ("%s", "Plugin-name"), ("%s", "Score"))
                ),
                options,
            )
        elif options.select != "":
...
https://DidierStevens.com"""
    ...
    oParser.add_option(
        "-a", "--all", action="store_true", default=False, help="display all the names"
    )
    oParser.add_option(
        "-e",
        "--extra",
    ...
    oParser.add_option(
        "--pluginoptions", type=str, default="", help="options for the plugin"
    )
    oParser.add_option(
        "-l",
        "--literalfilenames",
